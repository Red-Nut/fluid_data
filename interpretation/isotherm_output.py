...
from file_manager.fileModule import getFileSize
...
def CreateIsothermTemplate(id):
    try:
        ...
        fileLoc = f"well_data/{well.well_name}/"
        fileName = f"{well.well_name} - isotherms"
        fileExt = ".xlsx"
        filePath = root_folder + fileLoc + fileName + fileExt
        ...
        fileSize = getFileSize(fileLoc + fileName + fileExt)
        if fileSize is None:
            fileSize = 0
